...
for ($j = 4500; $j <= 5900; $j++) {
    ...
    $result = mysqli_query($connect, "SELECT weekly_sales, kmean, nbayes FROM testing_data WHERE id = $j");
...
if ($total_kmean > 0) {
    ...
    mysqli_query($connect, "UPDATE testing SET K_TP = $KTP, K_TN = $KTN, K_FP = $KFP, K_FN = $KFN, akurasi_kmean = $Pkmean, presisi_kmean = $presisi_kmean, recall_kmean = $recall_kmean WHERE id = 1");
} else {
...
if ($total_nbayes > 0) {
    ...
    mysqli_query($connect, "UPDATE testing SET N_TP = $NTP, N_TN = $NTN, N_FP = $NFP, N_FN = $NFN, akurasi_nbayes = $Pnbayes, presisi_nbayes = $presisi_nbayes, recall_nbayes = $recall_nbayes WHERE id = 1");
} else {
